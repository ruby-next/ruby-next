...
describe "ruby -ruby-next" do
  it "transform code in runtime when it's required" do
    run(
      "ruby -rbundler/setup -I#{File.join(__dir__, "../../lib")} -ruby-next -r #{File.join(__dir__, "fixtures", "beach.rb")} " \
      "-e 'puts beach(:k, 300)'"
  ...
  it "catches compile error for scripts and re-run them" do
    run(
      "ruby -rbundler/setup -I#{File.join(__dir__, "../../lib")} -ruby-next #{File.join(__dir__, "fixtures", "beach.rb")} k 10000"
    ) do |_status, output, _err|
